...
import type { ReportDetail } from "../../reports/types";
...
const StaffActionPanelInner = ({ report }: Props) => {
  ...
  const assignedToLabel = useMemo(() => {
    ...
    if (report.assignedTo?.name) return `Assigned to ${report.assignedTo.name}`;
...
const StaffActionPanel = ({ report }: Props) => {
  const isStaffStage = report.status === "VERIFIED" || report.status === "ASSIGNED" || report.status === "IN_PROGRESS";
  if (!isStaffStage) return null;

  return <StaffActionPanelInner report={report} />;
};
